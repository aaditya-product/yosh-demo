...
// estate. Each ties to a Section A beat; all are inert until that beat's
// screen exists (2.2 onward), same as an unbuilt rail item.
const quickActions: { label: string; icon: IconName }[] = [
  { label: 'Housekeeping', icon: 'cleaning' },
  { label: 'Order coffee', icon: 'coffee' },
  { label: 'Book a treatment', icon: 'spa' },
    ...
    <div className="flex h-full flex-col gap-xl overflow-y-auto p-xl">
      ...
      <div>
        ...
        <div className="mt-md flex flex-wrap gap-lg">
          {featured.map((s) => (
            <Card key={s.id} data-id={`${ID}/experience-${s.id}`} className="w-experienceCard p-lg">
